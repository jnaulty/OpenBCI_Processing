...
*/
 ...
 public void processMultiPerson(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
  ...
  FFT[] fftData) {              //holds the FFT (frequency spectrum) of the latest data
    ...
    // For all players... (2 for hackathon)
    for(int iPlayer = 0; iPlayer < NB_PLAYERS; iPlayer++)
    {
       Ichan = iPlayer;
       findPeakFrequency(fftData, Ichan);
        
       for(int iFreq = 0; iFreq < NB_FREQS; iFreq++)
       {
        //*********************************
        // Check Freqs.
        //*********************************

        if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[iFreq]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[iFreq])) // Player X - Freq X
        { 
          if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) 
          {
            detectedPeak[Ichan].threshold_dB = detection_thresh_dB;
            detectedPeak[Ichan].isDetected = true;
            isDetected[iPlayer] = true;
            //println("Player : " + (iPlayer+1) + " Freq : " + iFreq);
            // Do Something !
            Points[iPlayer] += (iFreq + 1) * 5;
          }
        }
       }     
      //*********************************
      // Print Stuff
      //*********************************
      if (isDetected[iPlayer]) 
      {
      //print some output
      /*println("EEG_Processing_User: " + "Player(" + (iPlayer+1) + ") : Chan " + (Ichan+1) + ", peak = " + detectedPeak[Ichan].rms_uV_perBin + " uV at " 
        + detectedPeak[Ichan].freq_Hz + " Hz with background at = " + detectedPeak[Ichan].background_rms_uV_perBin 
        + ", SNR (dB) = " + detectedPeak[Ichan].SNR_dB);*/
